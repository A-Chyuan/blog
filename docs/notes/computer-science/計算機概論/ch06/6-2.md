...
$Q:$ What kind of communication protocol is used in *Web-Based* Mail?
...
***Three Components of client***

- User interface
- Client control process
- Client data transfer process

***Three Components of Server***
...
$Q:$ Which of following item(s) is correct about Computer Networks?
...
![](./img/6.15.png ':figure Common scenario.')
...
- Alice prepares the email message using a User Agent (UA) program.
- Alice sends the message to her mail server
- The mail server at Alice's site stores the message in a queue awaiting delivery.
- The message is transmitted over the Internet from Alice's site to Bob's site using a Mail Transfer Agent (MTA).
- At Bob's site, the MTA receives the message from Alice's site.
- Bob's MTA places the message in a queue for delivery to Bob's user agent.
- The User Agent at Bob's site allows Bob to read the received message.
- Bob uses a Message Access Agent (MAA) clientto retrieve the message from an MAA server running on the second server.
- Bob accesses and reads the message retrieved by the MAA client.
...
$Q:$ What type of computer application is designed to facilitate the exchange of messages and files on a network?

---

$Ans:$ Bulletin Board System
...
- The current version is called SSH-2.
...
- A better solution is to distribute the information among many computers in the world. In this method, the host that needs mapping can contact the closest computer holding the needed information.
...
A **name space** can map each address to a unique name and is normally organized hierarchically. In a *hierarchical name space*, each name is made of several parts.
...
- **Generic domain**: `.com`, `.edu`, `.gov`, `.org`, ...
- **Country domain**: `.tw`, `.us`, `.jp`, ...
- **Inverse domain**:  
    However, due to the rapid growth of the Internet, it became extremely difficult to keep track of the inverse domains, which could be used to <mark>find the name of a host when given the IP address.</mark> The inverse domains are now *deprecated*.
...
A decentralized P2P network does not depend on a centralized directory system. In this model, peers arrange themselves into an overlay network, which is a logical network made on top of the physical network.
