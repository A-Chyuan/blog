...
Intuitively an array is a set of pairs, $\text{<index, value>}$, such that each index that is defined has a value associated with it. In mathematical terms, we call this a **correspondence** or a **mapping**. However, when considering an ADT we are more concerned with the operations that can be performed on an array.
...
\state $\textbf{objects:}$
\state A set of pairs <index, value> where for each value of index there is a value \\
...
\state for all A $\in$ Array, $\quad$ i $\in$ index, $\quad$ x $\in$ item, $\quad$ j, size $\in$ integer

\state \\

\function{Create}{j, list}
    \return an array of j dimensions \textbf{where}
    \state $\hspace{2em}$ list is a j-tuple whose ith element is the the size of the ith dimension.
    \state $\hspace{2em}$ Items are undefined.
\endFunction
...
- $\text{Store}$ accepts an array, an index, and an item, and returns the original array augmented with the new $\text{<index, value>}$ pair.
